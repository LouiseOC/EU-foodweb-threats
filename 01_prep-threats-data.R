...
# from eea website 
raw <- read.csv("data/European_Red_List_2017_December.csv")
...
spp.ID <- read.csv("data/species_codes_and_taxonomy.csv", sep = ";")

#clean data 
spp.ID$Species <- gsub(x=spp.ID$Species, pattern = " kl. ", replacement = " ")
spp.ID$Species <- gsub(x=spp.ID$Species, pattern = " ", replacement = "_")
spp.ID$Species <- gsub(x=spp.ID$Species, pattern = "_lorenzii", replacement = "") ## correct: P. sindianus lorenzii is the subspecies of P. s. that occurs in Europe - P. s. sindianus occurs in the Himalayas. 
...
## the following section builds a binary dataframe with threats
